<template>
  <Transition mode="out-in">
    ...
    <ErrorNotify v-else-if="errorRequest" @do-repeat-request="getProducts" class="wrapper__cart" />
    ...
    <main v-else class="content container">
      ...
      <section class="cart">
        <form class="cart__form form" action="#" method="POST">
          ...
          <div class="cart__block">
            ...
            <template v-if="numberProducts">
              <router-link :to="{ name: 'order' }" v-slot="{ navigate }" custom>
                <button type="submit" class="cart__button button button--primery" @click="navigate">
                  Оформить заказ
                </button>
              </router-link>
            </template>
        ...
        <NotifyMessage :isVisible="successfulRequestNotify" text="Товар удален из корзины" />
